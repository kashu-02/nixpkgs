...
let
  ...
  configFile =
      pkgs.writeText "snmptrapd.conf" ''
        ${lib.optionalString (cfg.disableAuthorization) "disableAuthorization yes"}
        ${lib.concatMapStringsSep "\n" (community: "authCommunity log,execute,net ${community}") cfg.authCommunity}
        ${cfg.extraConfig}
      '';
in
{
  options.services.snmptrapd = {
    ...
    authCommunity = lib.mkOption {
      ...
      '';
      example = [ "public" "private" "mySecureCommunity" ];
    };
  ...
  config = lib.mkIf cfg.enable {
    systemd.services.snmptrapd = {
      ...
      documentation = [ "man:snmptrapd(8)" "man:snmptrapd.conf(5)" ];
  ...
  meta.maintainers = with lib.maintainers; [ eliandoran ];
}
